...
// 19 nouveaux produits à ajouter
const newProducts = [
...
async function testConnection() {
  console.log('🔍 Test de connexion avec la clé secrète...');
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('✅ Connexion réussie avec la clé secrète !');
...
async function addProductsWithSecretKey() {
  console.log('🚀 Ajout massif avec clé secrète - contourne RLS...');
...
async function analyzeByCategory() {
  ...
  try {
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/products?select=category_id,categories(name)&order=category_id`,
      {
        headers: {
          'apikey': SUPABASE_SECRET_KEY,
          'Authorization': `Bearer ${SUPABASE_SECRET_KEY}`
    ...
    if (response.ok) {
      ...
      products.forEach(product => {
        const categoryName = product.categories?.name || 'Inconnue';
      ...
      console.log('❌ Erreur analyse catégories');
    }
    
  } catch (error) {
    console.log('❌ Exception analyse:', error.message);
  }
}

async function main() {
  console.log('🚀 AJOUT MASSIF DE PRODUITS AVEC CLÉ SECRÈTE');
  console.log('==============================================\n');
  
  const connected = await testConnection();
  
  if (connected) {
    const addedCount = await addProductsWithSecretKey();
    
    if (addedCount > 0) {
      await analyzeByCategory();
      
      console.log('\n🎯 Prochaines étapes recommandées:');
      console.log('1. 🔧 Exécuter supabase/advanced-features.sql pour les fonctionnalités avancées');
      console.log('2. 🌐 Rafraîchir localhost:3010 pour voir les nouveaux produits');
      console.log('3. 📱 Tester l\'application mobile avec les nouveaux produits');
    }
  } else {
    console.log('\n💡 Vérifiez la clé secrète ou utilisez le script SQL manuel');
  }
}
